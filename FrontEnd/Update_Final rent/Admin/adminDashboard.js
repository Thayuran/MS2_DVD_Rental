...
async function updateTotalEarnings(rentsUrl) {
    ...
    try {
        ...
        const rents = await response.json();
        if (rents) {
            const totalFines = rents.reduce((total, record) => {
                return total + (record.fine || 0);
            }, 0);
            totalFinesElement.textContent = `Rs. ${totalFines}`;
        } else {
            console.error("No rental data found");
        }
    } catch (error) {
        console.error("Failed to calculate total fines:", error);
    }
}
...
async function displayFrequentlyRentedDVDs(rentsUrl,apiUrl) {
    try {
        ...
        console.error("Failed to display top 5 frequently rented DVDs:", error);
    }
}
...
document.querySelectorAll(".navList").forEach(function(element) {
    ...
    customerdetails.forEach(cus => {
        ...
        deleteButton.addEventListener('click', () => {
            ...
            if (confirmation) {
                ...
                removeCustomerRow(actionCell.parentElement);
            }
        });
  ...
  function removeCustomerRow(customerRow) {
    customerRow.remove();
    alert("Customer removed from the customer Table");
}
    ...
    fetch('http://localhost:3000/adminNotification')
        ...
        .then(data => {
            ...
            data.forEach(notification => {
                ...
                // Determine the current status to control the dropdown and button visibility
                const isPending = notification.status === 'Pending';
                const isAccepted = notification.status === 'Accepted';
                const isRented = notification.status === 'Rented';
                ...
                row.innerHTML = `
                    ...
                    <td>${notification.user}</td>
                    <td>${notification.dvdName}</td>
                    <td id="status-${notification.id}">${notification.status}</td>
                    <td>${new Date(notification.date).toLocaleDateString('en-GB', {
                        month: '2-digit', day: '2-digit'
                    })}</td>
                    <td>
                        <select 
                            onchange="changeStatus(this, '${notification.id}', '${notification.userId}', '${notification.dvdId}')"
                            ${isRented || isRejected ? 'disabled' : ''}>
                            <option value="Accepted" ${isAccepted ? 'selected' : ''} ${isAccepted ? 'disabled' : ''}>Accepted</option>
                            <option value="Rented" ${isRented ? 'selected' : ''} ${isPending ? 'disabled' : ''}>Rented</option>
                            <option value="Rejected" ${isRejected ? 'selected' : ''} ${isPending ? '' : 'disabled'}>Rejected</option>
                        </select>
                    </td>
                    <td>
                        <button 
                            id="accept-${notification.id}" 
                            onclick="acceptRequest('${notification.id}')"
                            ${!isPending ? 'disabled style="display:none;"' : ''}>
                            Accept
                        </button>
                    </td>
                `;
...
function changeStatus(dropdown, notificationId, userId, dvdId) {
    const selectedStatus = dropdown.value;
    const statusCell = document.getElementById(`status-${notificationId}`);

    fetch(`http://localhost:3000/adminNotification/${notificationId}`, {
        method: 'PATCH',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    .then(updatedNotification => {
        ...
        statusCell.textContent = updatedNotification.status;

        // Adjust dropdown options and disable if status is "Rented" or "Rejected"
        if (updatedNotification.status === 'Accepted') {
            dropdown.querySelector('option[value="Accepted"]').disabled = true;
        } else if (updatedNotification.status === 'Rented' || updatedNotification.status === 'Rejected') {
...
function acceptRequest(notificationId) {
    ...
    .then(updatedNotification => {
        ...
        // Hide the "Accept" button and enable the "Rented" option in dropdown
        document.getElementById(`accept-${notificationId}`).style.display = 'none';

        // Find the dropdown and enable the "Rented" option
        const dropdown = statusCell.parentElement.querySelector('select');
        dropdown.querySelector('option[value="Rented"]').disabled = false;
    })
...
// 
            ...
            returnButton.addEventListener('click', () => {
                showConditionModal(record,returnButton);
            });
        });
    } catch (error) {
...
// 23.9
function showConditionModal(record,returnButton) {
    ...
    conditionForm.addEventListener('submit', async function (event) {
        ...
        if (returnDateObj > dueDateObj) {
            const delayDays = Math.ceil((returnDateObj - dueDateObj) / (1000 * 60 * 60 * 24));
        ...
        if (!dvdData) {
            alert("DVD not found in the inventory.");
            return;
        }
        ...
        alert("DVD return processed and inventory updated successfully!");
    });
}
// 
...
//update the return dvd copies

async function returnDVD(rentalId, dvdId) {
    try {
        ...
        if (rentalData.returnDate) {
            alert("This DVD has already been returned.");
            return;
        }
        ...
        console.error("Error updating rental and DVD data:", error);
...
//     if (!searchInput) {
//         alert('Please enter a Customer ID or DVD ID');
//         return;
//     }
...
//         const response = await fetch(`http://localhost:3000/rentals`);
//         if (!response.ok) throw new Error('Unable to show rental data');
        
//         const rentals = await response.json();
        

//         const filteredRentals = rentals.filter(rental => 
//             rental.customerId === searchInput || rental.dvdId === searchInput
//         );

//         if (filteredRentals.length === 0) {
//             alert('No rental history found for the given ID');
//             document.getElementById('rentalTable').style.display = 'none';
//             document.getElementById('generateReportBtn').disabled = true;
//             return;
//         }

//         displayRentalHistory(filteredRentals);
//     } catch (error) {
...
async function searchRentalHistory() {
    const searchInput = document.getElementById('searchInput').value.trim();
    if (!searchInput) {
        alert('Please enter a Username or DVD ID');
        return;
    }
    ...
    try {
        ...
        if (searchInput.startsWith('dvd_')) {
           
            await fetchRentalsByDvdId(searchInput);
        } else {
           
            const userResponse = await fetch(`${usersUrl}?name=${searchInput}`);
            ...
            userId = users[0].id;
            await fetchRentalsByUserId(userId);
        }
    } catch (error) {
        alert('Error fetching data: ' + error.message);
    }
...
async function fetchRentalsByUserId(userId) {
    try {
        ...
        if (rentals.length === 0) {
            alert('No rental history found for the given User ID');
            document.getElementById('rentalTable').style.display = 'none';
            document.getElementById('generateReportBtn').disabled = true;
            return;
        }
...
async function fetchRentalsByDvdId(dvdId) {
    try {
        const response = await fetch(`${rentsUrl}?dvdId=${dvdId}`);
        const rentals = await response.json();

        if (rentals.length === 0) {
            alert('No rental history found for the given DVD ID');
            document.getElementById('rentalTable').style.display = 'none';
            document.getElementById('generateReportBtn').disabled = true;
            return;
        }
...
function displayRentalHistory(rentals) {
    ...
    rentals.forEach(rental => {
        ...
        row.innerHTML = `
            ...
            <td>Rs.${rental.advance}</td>
            <td>Rs.${rental.fine || 0}</td>
            <td>${rental.payAction}</td>
        `;
        tableBody.appendChild(row);
    });
    
    document.getElementById('rentalTable').style.display = 'table';
    document.getElementById('generateReportBtn').disabled = false;
}

function generateReportAndPrint() {
    const rentalTable = document.getElementById('rentalTable').outerHTML;
    const printWindow = window.open('', '', 'width=800,height=600');
    printWindow.document.write('<html><head><title>Rental History Report</title></head><body>');
    printWindow.document.write('<h2>Rental History Report</h2>');
    printWindow.document.write(rentalTable);
